...
description: Hermes + Claude Code 协作协议 v2.0（本项目实例版）——通过 .collab/ 共享文件实现任务分配、状态同步和并发会话隔离（文件域锁 + 会话注册 + 认领原子化）。通用协议见独立仓库 qingcheng66/know-each-other.git，本文件为 blog 项目的实例化配置。
version: 2.0.1-blog
user-invocable: true
---

# Know Each Other — 协作协议 v2.0（blog 项目实例版）

通过 `.collab/` 目录下的文件实现 Agent 间任务交接与**并发隔离**，不需要 API、数据库或中心调度。

> 本文件 = 通用协议（见独立仓库 know-each-other.git）+ **本项目实例化配置**（文件域清单、会话命名、REQ 映射）。
...
## 本项目文件域（锁的粒度，针对 blog 项目结构定义）

| 域 | 覆盖文件 | 典型任务 |
|----|---------|---------|
| `styles` | globals.css、tailwind 配置 | REQ-018（草叶）、REQ-019（动效分级） |
| `layout` | layout.tsx、根组件 | REQ-018、REQ-019 |
| `content-data` | src/lib/content.ts、content/*.json | REQ-013（音乐迁移）、REQ-017（相册分组） |
| `components` | src/components/ 各组件 | REQ-016（Hero 口号）、REQ-013 |
...
**本项目冲突热点（历史教训）**：
- REQ-018 + REQ-019 都改 `globals.css` + `layout.tsx` — 这两个任务不能并行
- REQ-014（lint 清理）碰全仓 — 单独排在空闲会话做，不与任何任务并行
...
会话名由用户指定（本项目惯用 claude-a / claude-b），写进注册文件。
...
- 通用协议更新以独立仓库 know-each-other.git 为准，本项目实例化配置见本文件顶部「文件域」表
